/**
 * Weekly report generation -- aggregates health, meals, supplements,
 * and exercise data for the current or past weeks.
 *
 * Ported from src/api/routes/weekly-report.ts
 */
...
export async function getCurrentWeeklyReport(): Promise<WeeklyReport | null> {
  try {
    const now = new Date();
