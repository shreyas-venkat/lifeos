  ...
  try {
    const rows = await query(
      `SELECT s.id AS supplement_id, s.name, s.default_dosage, s.unit, s.time_of_day,
       ...
       LEFT JOIN lifeos.supplement_log sl ON s.id = sl.supplement_id AND sl.log_date = (NOW() AT TIME ZONE 'America/Edmonton')::DATE
       WHERE s.active = true
  ...
  try {
    await query(
      `INSERT INTO lifeos.supplement_log (id, supplement_id, taken, log_date, time_of_day)
       VALUES ($1, $2, true, (NOW() AT TIME ZONE 'America/Edmonton')::DATE, $3)
