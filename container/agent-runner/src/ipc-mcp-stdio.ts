...
server.tool(
  ...
  "Send a message to the user or group immediately while you're still running. Use this for progress updates or to send multiple messages. You can call this multiple times. Use target_channel to send to a specific Discord channel instead of the current one.",
  {
    ...
    target_channel: z
      ...
      .optional()
      .describe(
        '(Main group only) JID of the channel to send to, e.g. "dc:1487897174527311944" for #meals. Defaults to current channel.',
      ),
  },
  async (args) => {
    const targetJid =
      isMain && args.target_channel ? args.target_channel : chatJid;
...
\u2022 once: Local time WITHOUT "Z" suffix (e.g., "2026-02-01T15:30:00"). Do NOT use UTC/Z suffix.`,
  {
    ...
    output_channel_jid: z
      .string()
      .optional()
      .describe(
        '(Main group only) JID of the Discord channel where task output should be delivered. Defaults to the target group channel. Use this to run a task in the main group but deliver output to a different channel like #reminders.',
      ),
    script: z
      .string()
      .optional()
    ...
    model: z
      ...
      .optional()
      .describe(
        'Claude model to use for the task. Use "haiku" for simple/cheap tasks, "sonnet" for complex tasks. If omitted, uses the default model.',
      ),
  },
  async (args) => {
    ...
    const outputJid =
      isMain && args.output_channel_jid ? args.output_channel_jid : targetJid;
